...
import static com.intrepid.studio.kernel.Constants.ABSOLUTE_RESOURCES_APINFO;
import static com.intrepid.studio.kernel.Constants.ABSOLUTE_RESOURCES_TEXTURE;
import static com.intrepid.studio.kernel.Constants.API_EXTENSION;
import static com.intrepid.studio.kernel.Constants.GROUP_TAG;
import static com.intrepid.studio.kernel.Constants.NEW_EXTENSION;
import static com.intrepid.studio.kernel.Constants.SAI_EXTENSION;
import static com.intrepid.studio.kernel.Constants.TEXTURE_EXTENSION;
import static com.intrepid.studio.kernel.Constants.WORKDIR_ANIMATION;
...
//							String dir = split[ split.length - 2 ];
	...
	private void createGroupAssets() {
		for( String groupTagName : mapGroupTagResource.keySet() ) {
			...
			if     ( areaTotal <= (   32 *   32 ) ) pixmapBaseSize =   32;
			else if( areaTotal <= (   64 *   64 ) ) pixmapBaseSize =   64;
			else if( areaTotal <= (  128 *  128 ) ) pixmapBaseSize =  128;
			else if( areaTotal <= (  256 *  256 ) ) pixmapBaseSize =  256;
			else if( areaTotal <= (  512 *  512 ) ) pixmapBaseSize =  512;
			else if( areaTotal <= ( 1024 * 1024 ) ) pixmapBaseSize = 1024;
			else if( areaTotal <= ( 2048 * 2048 ) ) pixmapBaseSize = 2048;
			else if( areaTotal <= ( 4096 * 4096 ) ) pixmapBaseSize = 4096;
			else{
		...
		for( String groupTagName : mapGroupTagResource.keySet() ) {
			...
			for( TagResource tagRes : groupTag.getResources() ) {
				...
				if( freeSpaceToDelete != null ) {
					...
					if( freeSpaceAtSide != null ) freeSpaceList.add( freeSpaceAtSide );
					if( freeSpaceAbove != null ) freeSpaceList.add( freeSpaceAbove );
				} else {
...
//			getFileHandle( outputApInfo ).writeString( toWrite, false );
	...
	public void startThread() {
		...
		System.out.println( Designs.createTagForService( "GENERATE ANIMATION PACK INFO SERVICE" ) );
	}
