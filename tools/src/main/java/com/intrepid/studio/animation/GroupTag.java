...
class GroupTag {
	...
	public AnimationPackInfo createAnimationPackInfo() {
		AnimationInfo[] animataionsInfo = new AnimationInfo[ tagResources.size() ];
		for( int i = 0; i < tagResources.size(); i++ ) {
			animataionsInfo[ i ] = tagResources.get( i ).getAnimationInfo();
		}
		
		return new AnimationPackInfo( animataionsInfo );
	}
