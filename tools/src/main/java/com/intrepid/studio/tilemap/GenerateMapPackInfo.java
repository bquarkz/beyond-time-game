...
import static com.intrepid.studio.kernel.Constants.ABSOLUTE_RESOURCES_MPINFO;
import static com.intrepid.studio.kernel.Constants.RESOURCES_TEXTURE;
import static com.intrepid.studio.kernel.Constants.WORKDIR_TILEMAP;
import static com.intrepid.studio.kernel.Constants.WORKDIR_TILEMAP_JSON_MAP;
...
public class GenerateMapPackInfo implements ThreadRunnable {
    ...
    private static final int PAD_SIZE                   = Studio.common.getConfiguration().getPadSize();
    private static final int TILE_SIZE                  = Studio.common.getConfiguration().getTileSize();
    private static final int TILESET_SIZE               = Studio.common.getConfiguration().getTilesetSize();
    ...
    public void startThread() {
        ...
        System.out.println( Designs.createTagForService( "GENERATE MAP PACK INFO SERVICE" ) );
    ...
    public boolean executeThread() {
        ...
        rootContent.foreachFileInRootDirectory( WORKDIR_TILEMAP_JSON_MAP, new FileExec() {
            ...
            public void runOver( final FileHandle fHandle ) {
                ...
                MapPackInfo mapPackInfo = null; 
                System.out.println( "### Creating MapPackInfo " );
                try {
                    mapPackInfo = createMapPackInfoFrom( content, json );
                } catch( Exception e ) {
                    System.out.println( Designs.createErrorCode( " >>>> build fail, error message:\n" + e.getMessage() ) );
                    return;
                }
                System.out.println( "### MapPackInfo done" );
                
                String output = ABSOLUTE_RESOURCES_MPINFO + "/" + filename;
    ...
    public MapPackInfo createMapPackInfoFrom( String content, Json json ) {
        ...
        if( tileIdIndexer == null ) exit( "MAP", tiledReader );
        if( shapeCollisionIndexerTmp == null ) exit( "PHISYCS", tiledReader );
        if( soloIndexerTmp == null ) exit( "SOLO TYPE", tiledReader );
        if( monstersIndexerTmp == null ) exit( "MONSTERS", tiledReader );
        if( ornamentsIndexerTmp == null ) exit( "ORNAMENTS", tiledReader );
    ...
    private void showStatistics( EntityPosition[] monsterPositions, EntityPosition[] ornamentPositions ) {
    ...
    private void printLoadingBar( int i, int max ) {
    ...
    private void exit( String type, TiledReader tiledReader ) {
